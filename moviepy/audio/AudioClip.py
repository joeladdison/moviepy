...
class AudioClip(Clip):
    ...
    def to_soundarray(self, tt=None, fps=None, quantize=False, nbytes=2,
        ...
        if (tt is None):
            if self.duration > max_duration:
                return stacker(
                    self.iter_chunks(fps=fps, quantize=quantize,
        ...
        """
            ...
            return stacker([self.to_soundarray(
                tt=ttc, buffersize=buffersize,
                fps=fps,quantize=quantize, nbytes=nbytes)
    ...
    def max_volume(self, stereo=False, chunksize=50000, progress_bar=False):
        ...
        for chunk in self.iter_chunks(
                chunksize=chunksize, progress_bar=progress_bar):
            maxi = max(maxi, abs(chunk).max())
            if stereo:
