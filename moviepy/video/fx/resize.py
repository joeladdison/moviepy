...
try:
    ...
    try:
        ...
        try:
            ...
            resizer = lambda pic, newsize: imresize(
                pic, map(int, newsize[::-1]))
...
def resize(clip, newsize=None, height=None, width=None, apply_to_mask=True):
    ...
    # From here, the resizing is constant (not a function of time),
    # size=newsize

    if clip.ismask:
        fl = lambda pic: 1.0*resizer(
            (255 * pic).astype('uint8'), newsize)/255.0
