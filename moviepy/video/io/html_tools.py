...
def html_embed(clip, filetype=None, maxduration=60, rd_kwargs=None,
    ...
    **html_kwargs
    ...
    if "Clip" in str(clip.__class__):
        ...
        return html_embed(
            filename, maxduration=maxduration,
            rd_kwargs=rd_kwargs, center=center, **html_kwargs)

    filename = clip
    options = " ".join(["%s='%s'" % (str(k), str(v))
    ...
    if filetype is None:
        ...
        if ext == "gif":
            ...
            raise ValueError("No file type is known for the provided file. "
                             "Please provide argument `filetype` (one of "
                             "'image', 'video', 'sound') to the "
    ...
    if filetype in ['audio', 'video']:
        ...
        if duration > maxduration:
            raise ValueError(
                "The duration of video %s (%.1f) " % (filename, duration) +
                "exceeds the 'max_duration' "
                "attribute. You can increase 'max_duration', but note that "
                "embedding large videos may take all the memory away !")
