...
class FramesMatches(list):
    ...
    def filter(self, cond):
        """
        Returns a FramesMatches object obtained by filtering out the
        FramesMatch which do not satistify the condition ``cond``. ``cond``
    ...
    def from_clip(clip, dist_thr, max_d, fps=None):
        ...
        with a duration of 1.5s or more into a GIF:
        ...
        for (t, frame) in clip.iter_frames(with_times=True, progress_bar=True):
            ...
            matching_frames += [(t1, t, F[t1][t]['min'], F[t1][t]['max'])
                                for t1 in F
    ...
    def select_scenes(self, match_thr, min_time_span, nomatch_thr=None,
        ...
        for start, ends_distances in starts_ends:
            ...
            end, d_min, d_max = [e for e in great_long_matches
                                 if e[0] == end][0]
